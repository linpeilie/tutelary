...
public class TutelaryAgent {
    ...
    private static synchronized void main(String args, final Instrumentation instrumentation) {
        try {
            ...
            LOG.info("tutelaryAgentProperties : {}, classloader : {}", tutelaryAgentProperties, tutelaryAgentProperties.getClass().getClassLoader());
            Thread.currentThread().setContextClassLoader(agentClassLoader);
            start.invoke(null, instrumentation, JSONUtil.toBean(args, agentClassLoader.loadClass(TutelaryAgentProperties.class.getName())));
        } catch (Exception e) {
